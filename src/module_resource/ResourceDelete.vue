            ...
            <div class="modal-content">
                <div class="block block-themed block-transparent remove-margin-b">
                    <div class="block-header bg-danger">
                        ...
                        <div class="modal-title">删除资源</div>
                    </div>
                    <div class="block-content" style="padding-bottom: 20px;">
                        确定删除资源 {{itemData.name}} 吗?
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        ok: function () {
            ...
            if (!_label.isBlank()) {
                let _this = this;
                this.$axios.post('resources/' + _label + '/delete').then(function (response) {
                    toastr.success("删除资源成功");
                    _this.$eventHub.$emit('resources.updated');
                }).catch(function (error) {
                    toastr.error("删除资源异常 [" + error + "]");
                });
            } else {
                toastr.error("资源名称不合法或为空");
            }
        }
    }
}
